...
- Development is a single 800 ms linear crossfade from the neutral code-drawn archive plate to the illustration. The retained 45 illustrations preserve every non-trace pixel and map only exact legacy aspect-trace pixels to the canonical palette; their thumbnails remain derived from the same art. A click or Space during development completes it and is consumed before any doorway or facsimile control.
...
Every `bc.threads.v2` definition includes a stable `concept_id`, authoritative
`owner`, 20–35 word rule, exact `suit`, `order`, and canonical lowercase
...
and list sizes no larger than 52. Unknown cards leak no rule or narrative copy.
