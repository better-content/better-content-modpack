...
| Logistics | Ungated roots and hidden post-craft tax | Visible Ratlantis components root Pretty Pipes, Sophisticated automation, Create requests, AE2, Little Logistics, and Rats; one Courier Lattice yields a 32-pipe starter network, Oratchalcum gates blank modules, and Arcane Logistics Cores gate all seven high modules | Ratlantis Logistics | Useful first-trip infrastructure with visible mid/high upgrades | recipes, custom hook | static contract + custom test + server runtime gate |
