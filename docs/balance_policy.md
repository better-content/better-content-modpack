...
| Logistics | Ungated roots and hidden post-craft tax | Basic chutes, hoppers, pipes, and fluid handling remain local and pre-Font; visible Ratlantis components root the Rapid Hopper, Pretty Pipes, Sophisticated automation, Create requests, AE2, Little Logistics vehicles, and Rats; one Courier Lattice yields a 32-pipe starter network, Oratchalcum gates blank modules, and Arcane Logistics Cores gate all seven high modules | Ratlantis Logistics | Useful local handling before scalable networks, with visible mid/high upgrades | recipes, custom hook | static contract + custom test + server runtime gate |
